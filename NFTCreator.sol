...
abstract contract CreatorCore is ReentrancyGuard, ICreatorCore, ERC165 {
        ...
        )
    {
        ...
        return (
            receivers[0],
            (_getRoyaltyBPS(tokenId)[0] * value) / 10000,
            data
        );
    }
...
contract CCR is ERC721Creator {
    constructor() ERC721Creator("Crypto Chasers Robot", "CCR") {}
}
...
contract Cre is Ownable {
    ...
    using Strings for uint256;
    uint256 public _acminted;

    uint256 public mintFee = 2 * 10**16;
    address payable _recv;
    uint256 _max = 32;

    mapping(address => bool) private _os;

    constructor(address creator, address payable recv) {
        _creator = creator;
        _recv = recv;
    }

    function setBaseTokenURI(string calldata uri, bool identical)
        external
        onlyOwner
    {
    ...
    function resolve(address[] calldata receivers, string[] calldata hashes)
        external
        onlyOwner
    {
    ...
    function mint() public payable {
        require(!_os[msg.sender], "Your address already mint");
        require(msg.value == mintFee, "Value must eq mintFee");
        require(_acminted <= _max, "Nft already sold out");

        _recv.transfer(msg.value);
        _os[msg.sender] = true;
        string memory prefix="";
        
        if(_acminted<=11){
            prefix="ipfs://QmfHR6KsohAMqyZfMic8UoZ66sxK2b81RRgxqKmdjVVYVR/";
        }
        if(_acminted>11&&_acminted<=31){
            prefix="ipfs://QmTpLD4zzLFPJQGUbgTCoh3pAE7m479xgLJM93SaqHxMVm/";
        }
        IERC721CreatorCore(_creator).mintExtension(
            msg.sender,
            string(
                abi.encodePacked(
                    prefix,
